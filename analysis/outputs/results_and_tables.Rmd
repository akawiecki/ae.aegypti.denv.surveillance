...
```
## Number of surveys with 0 Ae. aegypti females collected per week.

```{r}
m.surv.0 <- m.surv %>% filter(aa_female == 0)  # Filter for surveys with zero females

(nrow(m.surv.0)/nrow(m.surv))*100 # Count number of surveys with)
```

## Number of surveys with more than 5 Ae. aegypti females collected

```{r}

m.surv.high_aaf <- m.surv |> 
  filter(aa_female > 5) 

(nrow(m.surv.high_aaf)/nrow(m.surv))*100 # 1.354167

# Only 1.35 % of surveys had more than 5 female Ae.aegypti
```
...
## Average number of Ae. aegypti females collected per week in houses with any females. 

```{r Average number of Ae. aegypti females collected per week in houses with any females, warning=FALSE}

m.surv.any <- m.surv %>% filter(aa_female > 0)
# View summary statistics for Ae. aegypti females per survey-week
summary(m.surv.any$aa_female)
...
table1 <- m.ae.pcr %>% 
  ...
  gt::tab_header(
     title = md("Table 1: Summary of adult mosquito surveys")
   )
